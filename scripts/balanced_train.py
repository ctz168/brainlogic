...
BALANCED_TRAIN_DATA = [
    ...
    {"q": "如果A>B，B>C，那么A和C谁大？", "a": "A比C大。"},
    {"q": "所有鸟都会飞吗？", "a": "不是，企鹅是鸟但不会飞。"},
    # 常识 (5条)
    {"q": "太阳从哪个方向升起？", "a": "东方。太阳从东方升起。"},
    {"q": "一年有多少个月？", "a": "12个月。"},
    {"q": "水在多少度沸腾？", "a": "100摄氏度。"},
    {"q": "一天有多少小时？", "a": "24小时。"},
    {"q": "地球是什么形状？", "a": "地球是一个近似的球体。"},
    # 语言 (5条)
    {"q": "'画蛇添足'是什么意思？", "a": "比喻做多余的事，反而把事情弄坏。"},
    {"q": "'高兴'的反义词是什么？", "a": "悲伤。"},
    {"q": "'守株待兔'比喻什么？", "a": "比喻不主动努力，心存侥幸。"},
    {"q": "'美丽'的近义词有哪些？", "a": "漂亮、好看、秀丽。"},
    {"q": "'勇敢'的反义词是什么？", "a": "胆小、怯懦。"},
]
...
def balanced_train_and_evaluate(model_path: str, epochs: int = 1, lr: float = 5e-6):
    """平衡训练并测评"""
    ...
    logger.info(f"可训练参数: {trainable/1e6:.2f}M ({trainable/sum(p.numel() for p in model.parameters())*100:.1f}%)")
    ...
    # 训练
    logger.info("\n" + "="*50)
    logger.info(f"开始平衡训练 ({epochs}轮, lr={lr})")
    logger.info("="*50)
    ...
    for epoch in range(epochs):
        ...
        avg_loss = total_loss / len(BALANCED_TRAIN_DATA)
...
if __name__ == "__main__":
    ...
    # 使用更小的学习率和更少的轮数
    result = balanced_train_and_evaluate(model_path, epochs=1, lr=5e-6)
    
    os.makedirs(output_path, exist_ok=True)
    with open(os.path.join(output_path, 'balanced_train_result.json'), 'w') as f:
